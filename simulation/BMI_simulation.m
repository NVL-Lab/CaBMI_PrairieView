function [target_info_path, target_cal_ALL_path, fb_cal] = BMI_simulation(n_f_file, roi_data_file,  strcMask_file, ...
    ...
    folder_plot = fullfile(folder_save, 'plots'); 
    if not(isfolder(folder_plot))
        mkdir(folder_plot); 
    end
    %%sa
    ...
    load(n_f_file); 
    %n_f_file - contains matrix, neural fluorescence from baseline file, num_samples X num_neurons_baseline 
    baseActivity(:,isnan(baseActivity(1,:))) = [];
    baseActivity = baseActivity.'; %num_samples X num_neurons

    %baseActivity = baseActivity(1:18000,:)

    % baseActivity = baseActivity(1:1000, :); %debugging input data with nans... 
    %Assume variable is called baseActivity

    E1_temp = baseActivity(:,E1_base); 
    E2_temp = baseActivity(:,E2_base); 
    f = [E1_temp E2_temp]; 

    %Throw out prefix frames:
    E1_raw = baseActivity((tset.prefix_win+1):end,E1_base); 
    E2_raw = baseActivity((tset.prefix_win+1):end,E2_base); 
    ...
    save(fullfile(folder_save, 'strcMask.mat'), 'strcMask', 'E_base_sel', 'E_id'); 
    ...
    if(plot_raw_bool)
        ...
        im_path = fullfile(folder_plot, 'baseline_fraw.png'); 
        saveas(h, im_path); 
    end
    ...
    %         legend({'f0win', 'f0mean'})
            ...
            saveas(h, fullfile(folder_plot, 'f0.png')); 
        else
            %Plot for one neuron: 
            n_i = 1; 
            h = figure; hold on;
    ...
    if(plot_dff_bool)
        ...
        ylabel('dff'); 
        title('dff'); 
        im_path = fullfile(folder_plot, 'dff.png'); 
        saveas(h, im_path);
        ...
        ylabel('dff_z');    
        title('zscore dff'); 
        im_path = fullfile(folder_plot, 'dffz.png'); 
        saveas(h, im_path); 
    end
    ...
    if(plot_cov_bool)
        ...
        caxis([-0.2 0.5]); 
        title('neural cov'); 
        saveas(h, fullfile(folder_plot, 'cov_mat_baseline.png'))
        ...
        saveas(h, fullfile(folder_plot, 'cov_pca_baseline.png'))
    end
    ...
    saveas(h, fullfile(folder_plot, 'target_val_over_calibration.png')); 
    ...
    % saveas(h, fullfile(folder_save, 'reward_per_frame_over_calibration.png')); 
    ...
    ylabel('Audiory Freq'); 
    vline(T); 
    saveas(h, fullfile(folder_plot, 'cursor2freq.png')); 
    ...
    saveas(h, fullfile(folder_plot, 'base_freq_hist.png')); 
    ...
    saveas(h, fullfile(folder_plot, 'cursor_dist_T.png')); 
    ...
    % % end
    % 
    % saveas(h, fullfile(folder_plot, 'neural_hit_constraints.png')); 
    ...
    saveas(h, fullfile(folder_plot, 'PSTH_locked_to_hit_baseline.png')); 
    ...
    save_path = fullfile(folder_save, ['target_calibration_ALL_' date_str '.mat']); 
    target_cal_ALL_path = save_path; 
    save(save_path); 

    %2)Just the target parameters for running BMI
    target_info_file = ['BMI_target_info_' date_str '.mat'];
    save_path = fullfile(folder_save, target_info_file); 
