function fAccessvars = dex_lsm(matin,framerate,duration,name,path);
...
%         %modified by nuria (comment don't erase please)
%         if (exist ('roitraces_sodcit.mat')) == 0
%             roi_s = roiTraces;
%         else
%             load ('roitraces_sodcit.mat')
%             roi_s(:,end+1) = roiTraces (:,1);
%         end
%         save ('roitraces_sodcit','roi_s')
        %save(['ROI_' name],'roiTraces');
        %end of modified nuria
