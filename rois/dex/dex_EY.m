function fAccessvars = dex_EY(matin,framerate,duration,name_n,expnum,trial,pathname);

% [output,IM_MASTER]=Im_Load_EY (name_n,expnum,trial,1);
...
%%%%% THIS PART DRAW THE ROI regions _ EY
...
fAccessvars = @accessvars;
    ...
    function plotRoiTraces;
        ...
        p=pathname;
        cd(p);
        save([pathname, 'ROI_', name_n,'_E',expnum,'_',num2str(trial)],'roiTraces');
        ...
        if ~ishandle(h.RoiPlotAx)
            ...
            %Time scale while plotting is determined by timeint
            timint=1;
            set(gca,'XTick',(0:(timint/framerate):(duration/framerate)))
            set(gca,'XTickLabel',(0:timint:duration));
            ...
            title(['date=',name_n,'  -exp#',expnum,'  -sweep#',num2str(trial),'    ',pathname],'fontsize', 12)
